# State machine — `/product-foundation` v0.6.0 (`.state.json` v6)
...
- **`idea`** — verbatim user input from `/product-foundation "<idea>"`. Immutable.
...
2. Skill invokes `AskUserQuestion` with 3 options:
   ...
   - **`abort`** → exit cleanly. Sets `flags.from_step` = current step for resume hint. Prints `Run /product-foundation "<idea>" --from-step=<NN> --out=<same-path>` to resume.
...
```bash
/product-foundation "ERP para salões de beleza" --from-step=09 --out=/tmp/dogfood-erp
```

Behavior:

1. Phase 0 reads `.state.json` from `<out-dir>/docs/`.
2. **Validates `version`** — must be `6`. If `version == 5`, abort with `state v5 found — older /product-foundation run; clear --out dir or run fresh /product-foundation`. If `version == 4`, abort with `state v4 found — older /product-foundation run; clear --out dir or run fresh /product-foundation`. If `version == 3`, abort with `state v3 found — older /product-foundation run; clear --out dir or run fresh /product-foundation`. If `version < 3` (v1 or v2), abort with `state v<N> found — older /product-foundation run; clear --out dir or run fresh /product-foundation`. Conservative: refuse to silently upgrade an older state file, because (1) v5→v6 adds the concept gate + form-variant dispatch (a v5 resume would mis-track gates/iterations); (2) v4→v5 reshapes Phase 4/5 (a v4 resume into Phase 4 would expect the deleted screen-writer fan-out); (3) v3→v4 changed artifact paths (NN-prefix dropped); (4) v2→v3 changed step numbering.
