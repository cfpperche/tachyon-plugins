...
Thin wrapper over `scripts/audit.sh`, which runs `osv-scanner` from PATH and shapes its output. The script is the engine; this skill decides when to run it and how to surface the result.
...
2. **Invoke the script.** It ships inside this skill, so run it from **this skill's own directory**:
   ...
   The script itself is runtime-neutral; it resolves osv-scanner from PATH regardless of which runtime it
...
3. **Surface the result** — relay the script's report. The first line is `dep-audit: status=<clean|findings|unavailable|failed>`:
   ...
   - **`unavailable`** — osv-scanner is not on PATH. Relay the install hint the script prints. **Do not treat this
     as "clean"**: nothing was scanned, so the absence of findings means nothing.
...
- Two tools must be on PATH: `osv-scanner` (the engine) and `jq` (the script parses the engine's JSON with it).
  Tachyon installs neither — the manifest names both in `requires` and the operator installs them. See the README.
