#!/usr/bin/env bash
# sdd-dogfood.sh — run a spec's declared dogfood command(s). PREVIEW BY DEFAULT.
#
# A spec opts in to headless dogfood by declaring one or more
#   **Dogfood:** `<command>`
...
[ -n "$SPECS_ROOT" ] || { printf '%s: no docs/specs under the workspace root (%s) — nothing to dogfood\n' "$SELF" "$ROOT" >&2; exit 64; }
...
extract_cmds() {
  local f="$1"
  [ -f "$f" ] || return 0
  grep -nE '^\*\*Dogfood:\*\*[[:space:]]*`' "$f" 2>/dev/null \
    | sed -E 's/^[0-9]+:\*\*Dogfood:\*\*[[:space:]]*`([^`]*)`.*/\1/'
}
...
if [ -z "$CMDS" ]; then
  if [ "$OUT_JSON" -eq 1 ]; then
    printf '{"status":"no-dogfood-declared","spec":"%s","commands":[],"passed":0,"failed":0,"declared":false,"ran":false}\n' \
      "$(json_escape "$SPEC_NAME")"
  elif [ "$QUIET" -eq 0 ]; then
    printf '%s: no dogfood command declared in %s — nothing to run (declare a **Dogfood:** `<cmd>` line)\n' "$SELF" "$REL_SPEC"
  fi
  exit 2
fi
...
'
    ...
    printf '%s: preview — %s declares %s dogfood command(s) (source: %s). NOTHING was run.\n' "$SELF" "$REL_SPEC" "$_n" "$SRC"
...
GIT_SHA="$(cd "$ROOT" && git rev-parse HEAD 2>/dev/null || printf 'unknown')"
PASSED=0
FAILED=0
RESULT_LINES=""
JSON_ITEMS=""
...
grep -qE '^## Dogfood log' "$NOTES_MD" || printf '\n## Dogfood log\n' >> "$NOTES_MD"
BLOCK_FILE="$(mktemp)"
TMP_NOTES="$(mktemp)"
trap 'rm -f "$BLOCK_FILE" "$TMP_NOTES"' EXIT
{
  printf '\n### %s — %s (%d/%d) — source: %s — commit: %s\n' "$TS" "$OVERALL" "$PASSED" "$TOTAL" "$SRC" "$GIT_SHA"
  printf '%s' "$RESULT_LINES"
} > "$BLOCK_FILE"

if ! awk -v block="$BLOCK_FILE" '
  /^##[[:space:]]+Dogfood log[[:space:]]*$/ { in_dogfood=1; print; next }
  in_dogfood && !inserted && /^##[[:space:]]/ {
...
' "$NOTES_MD" > "$TMP_NOTES" || ! mv "$TMP_NOTES" "$NOTES_MD"; then
  printf '%s: failed to append dogfood log to %s\n' "$SELF" "$REL_SPEC/notes.md" >&2
  exit 1
fi

if [ "$OUT_JSON" -eq 1 ]; then
  printf '{"status":"%s","spec":"%s","source":"%s","commit":"%s","commands":[%s],"passed":%d,"failed":%d,"declared":true,"ran":true}\n' \
    "$OVERALL" "$(json_escape "$SPEC_NAME")" "$SRC" "$(json_escape "$GIT_SHA")" "$JSON_ITEMS" "$PASSED" "$FAILED"
elif [ "$QUIET" -eq 0 ]; then
