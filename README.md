...
| [`visual-qa`](./visual-qa) | **Advisory Visual QA** for a web UI a worktree changed. Drives the page via `agent-browser`, screenshots declared routes, judges against YOUR design-intent anchor (not a pixel baseline), and attaches verdict + screenshots to the worktree evidence channel. Never gates a merge; requires the `agent-browser` plugin alongside. | claude · codex |
