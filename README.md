...
| [`agent-screen`](./agent-screen) | **OS-level screenshots** for non-web Visual QA and installed-app dogfood. Captures explicit PNG evidence from the real desktop via a Linux/WSLg X11 backend and fails closed when no display/backend exists. V1 is screenshot-only; screen recording is deferred. | claude · codex |
