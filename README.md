...
|---|---|---|
| [`sdd`](./sdd) | Spec-driven development — scaffolds + progresses `docs/specs/NNN-<slug>/{spec,plan,tasks,notes}.md` (`new`/`plan`/`tasks`/`list`) with same-clone worktree-safe numbering, then closes the loop: `verify` re-runs a spec's declared check, `dogfood` runs a declared headless dogfood command (both preview-by-default), and `close` audits shipped specs for closure debt and dogfood proof or opt-out. | claude · codex |
